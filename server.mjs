...
// In-memory cache + Supabase persistent store
const sessions = new Map();

async function sbSessionGet(sid) {
  if (!config.supabaseUrl || !config.supabaseKey) return null;
  try {
    const r = await fetch(`${config.supabaseUrl}/rest/v1/bt_sessions?id=eq.${sid}&select=data`, {
...
async function sbSessionSave(sid, data) {
  if (!config.supabaseUrl || !config.supabaseKey) return;
  try {
    await fetch(`${config.supabaseUrl}/rest/v1/bt_sessions`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ id: sid, data, updated_at: new Date().toISOString() })
    });
...
async function getSession(req, res) {
  ...
  let isNew = false;
  if (!sid) { sid = randomBytes(24).toString("base64url"); isNew = true; }

  if (!sessions.has(sid)) {
    // Try loading from Supabase first
    const persisted = await sbSessionGet(sid);
    if (persisted) {
      sessions.set(sid, persisted);
    } else {
  ...
  if (isNew) {
    res.setHeader("Set-Cookie", `bt_session=${sid}; Path=/; HttpOnly; SameSite=None; Secure`);
  }
...
async function saveSession(session) {
  ...
  await sbSessionSave(session.id, session);
}
...
async function onshapeCallback(req, res, session, url) {
  ...
  // Persist session to Supabase so it survives server restarts
