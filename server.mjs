...
function onshapeStart(req, res, session, url) {
  ...
  // Pass through embedded/document params so we can redirect back after login
  const returnTo = url.searchParams.get("returnTo") || "/public/panel.html";
...
async function onshapeCallback(req, res, session, url) {
  ...
  if (state !== session.oauthState)    return json(res, 400, { error: "Invalid state" });
  ...
  // Fetch user info
  try {
    const me = await onshapeGet(session, "/api/users/sessioninfo");
    session.user = { name: me.name, email: me.email, id: me.id };
  } catch (e) {
    console.warn("Could not fetch user info:", e.message);
  }

  redirect(res, session.returnTo || "/public/panel.html");
}
